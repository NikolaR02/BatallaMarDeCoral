...
public class PestanaAzul extends Stage {
        ...
        Timeline moverse = new Timeline(new KeyFrame(Duration.seconds(1), e -> {
            for (Barco barco : this.control.getBarcos()) {
                ...
                if (barco.getNombre().equals("acorazado") && barco.getEquipo().equals("Azul")) {
                    textoVidaAcorazado.setText(String.valueOf(barco.getVida()));
    ...
    private void cargarInterfaz() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("barcosAzules.fxml"));
            ...
            textoVidaAcorazado = (Label) loader.getNamespace().get("vidaAcorazado");
            textoVidaAcorazado.setText("120");
